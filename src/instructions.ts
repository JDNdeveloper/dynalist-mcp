...
- No URL for folders.
- Tools accept file_id and item_id, not URLs. Extract file_id from the /d/ path segment \
and item_id from the #z= fragment.
...
- **File vs item management**: File tools (create_document, move_document, etc.) operate on the file tree. \
Item tools (insert_items, edit_items, etc.) operate within a document. Do not confuse file IDs with item IDs.
...
- **IMPORTANT: The preview MUST show ALL affected locations.** For moves, show both where items are \
...
- When checking (completing) a parent item, do not check its children unless the user explicitly asked. \
Dynalist visually greys out descendants of checked items.
`;
