...
- Read with `max_depth: 1`. Verify non-leaf depth-1 nodes show `depth_limited: true`, `child_count` populated, `children` omitted. Leaf nodes at depth 1 omit both `child_count` and `children`.
