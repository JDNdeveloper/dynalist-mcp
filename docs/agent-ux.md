...
To sidestep this, the server delivers its full instructions through a `get_instructions` tool. The native `instructions` field carries a short directive telling the agent to call `get_instructions` before any other tool, and that tool returns the full INSTRUCTIONS body. The directive is short enough to survive truncation. Every non-meta tool description starts with a similar one-sentence requirement, so an agent that sees any operational tool also sees the `get_instructions` requirement before the tool-specific schema. The generated tools reference strips this agent-only prefix so the docs stay focused on tool behavior. As of April 2026, Codex does not reliably surface the MCP server instructions field and can omit `get_instructions` from tool-search results, so each operational tool carries the `get_instructions` requirement itself.
...
- **Checked items.** Completed items use strikethrough syntax (`~~text~~`) to represent Dynalist's visual treatment.
- **Fenced blocks.** Every tree must be enclosed in a fenced code block. Codex CLI strips indentation from unfenced `•` lines but preserves it inside fenced blocks. The tradeoff is that inline Markdown, including strikethrough and links, appears as literal text inside the block.
...
The instructions specify `•` (U+2022, unicode bullet) as the bullet character and explicitly forbid `-`, `*`, and `+` as Dynalist tree bullet markers. This restriction does not apply to item text, diff prefixes, or output outside Dynalist tree blocks. This is a deliberate choice driven by weak-model compatibility testing with Haiku.
